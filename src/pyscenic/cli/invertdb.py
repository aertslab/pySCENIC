...
def create_argument_parser():
    ...
    parser.add_argument(
        '-o', '--outputdir', type=str, default=os.getcwd(), help='Output directory (default: current directory).'
    )
    parser.add_argument(
        '-n',
        '--topn',
...
def convert(out_folder, in_fnames, topn):
    for fname in in_fnames:
        ...
        InvertedRankingDatabase.invert(
            opendb(fname=fname.name, name=name), os.path.join(out_folder, "{}.inverted.feather".format(name)), topn
        )
